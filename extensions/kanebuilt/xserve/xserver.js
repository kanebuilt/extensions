...
const roomStore = new RoomStore(process.env.XSERVER_ROOMS_DB_PATH || DEFAULT_ROOMS_DB_PATH);
roomStore.load();
...
const log = {
  info: (msg) =>
    console.log(`${chalk.gray(`[${getTimestamp()}]`)} ${chalk.cyan.bold('[INFO]')}    ${msg}`),
  success: (msg) =>
    console.log(`${chalk.gray(`[${getTimestamp()}]`)} ${chalk.green.bold('[SUCCESS]')} ${msg}`),
  warn: (msg) =>
    console.log(`${chalk.gray(`[${getTimestamp()}]`)} ${chalk.yellow.bold('[WARN]')}    ${msg}`),
  error: (msg) =>
    console.log(`${chalk.gray(`[${getTimestamp()}]`)} ${chalk.red.bold('[ERROR]')}   ${msg}`),
  verbose: (msg) =>
    console.log(`${chalk.gray(`[${getTimestamp()}]`)} ${chalk.magenta.bold('[VERBOSE]')} ${msg}`),
};
  ...
  const timeout = setTimeout(() => {
    try {
      ...
      const safeErrorMessage = String(error?.message ?? 'Unknown error')
        ...
        .replace(/[^\x20-\x7E]/g, '')
        .slice(0, 200);
      log.error(`Failed room expiry cleanup | Room: "${roomName}" | Error: ${safeErrorMessage}`);
    }
  }, ROOM_EXPIRY_MS);
...
wss.on('connection', (ws, req) => {
  ...
  const clientVersion = String(connectionUrl.searchParams.get('xserveVersion') ?? '').trim();
  ...
  ws.on('message', (rawMessage) => {
    ...
    if (rateWindowCount > MAX_MESSAGES_PER_SECOND) {
      ...
      log.warn(`Rate limit triggered for Client ${myClientId}; ignoring input until cooldown ends`);
      return;
    }
      ...
      switch (data.type) {
        case 'create': {
          ...
          if (activeRoom && activeRoom.host.readyState === WebSocket.OPEN) {
            log.warn(`Client ${myClientId} attempted to create existing room: "${sanitizedRoom}"`);
        ...
        case 'join': {
          ...
          if (!targetRoom || targetRoom.host.readyState !== WebSocket.OPEN) {
            log.warn(`Client ${myClientId} attempted to join offline room: "${sanitizedRoom}"`);
            ws.send(JSON.stringify({ type: 'error', message: 'Room is currently offline.' }));
            return;
          }
          ...
          if (
            ...
            log.warn(`Client ${myClientId} failed authentication for room: "${sanitizedRoom}"`);
          ...
          log.success(`Client joined room | Client ID: ${myClientId} | Room: "${currentRoom}"`);
          break;
        }
        ...
        case 'send_to_client': {
          ...
          if (isHost && currentActiveRoom) {
            ...
            if (targetClient && targetClient.readyState === WebSocket.OPEN) {
              ...
              log.warn(`Host attempted to send to invalid client | Target: ${sanitizedTarget}`);
            }
          }
          break;
        }
        ...
        case 'broadcast': {
          ...
          if (isHost && currentActiveRoom) {
            ...
            log.verbose(`Broadcast sent | Room: "${currentRoom}" | Recipients: ${broadcastCount}`);
          }
          break;
        }
        ...
        default:
          log.warn(`Unknown message type received | Client ID: ${myClientId} | Type: ${data.type}`);
      }
    } catch (e) {
      log.error(`Malformed message received from Client ${myClientId} | Error: ${e.message}`);
    }
  });
    ...
    if (currentRoom && currentActiveRoom) {
      if (isHost) {
        ...
        if (currentActiveRoom.host.readyState === WebSocket.OPEN) {
          currentActiveRoom.host.send(JSON.stringify({ type: 'client_left', id: myClientId }));
        }
