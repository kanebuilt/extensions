...
(function (Scratch) {
  ...
  class DrivesExtension {
    constructor() {
      this._assertRuntimeSupport();
      this.volumes = {};
      this.lastError = JSON.stringify({ status: 'success' });
      // Toggle verbose logging for Drives (no-op when false)
      this.DrivesLogEnabled = false;
      ...
      this._ready = this._initVolumes().catch((e) => {
        this.lastError = JSON.stringify({
          status: 'error',
          code: 'INTERNAL_ERROR',
          message: 'Failed to initialize drives: ' + (e.message || String(e)),
        });
        this._warn('Drives initialization failed:', e);
      });
    }
      ...
      return {
        ...
        blocks: [
          ...
          },
          {
            ...
            text: Scratch.translate('list mounted drives'),
          ...
          {
            blockType: Scratch.BlockType.LABEL,
            text: Scratch.translate('Path & Directory'),
          },
          {
            opcode: 'listFiles',
            blockType: Scratch.BlockType.REPORTER,
    ...
    _assertRuntimeSupport() {
      ...
      try {
        if (
          ...
          unsupportedReason =
            'Drives requires a secure context (HTTPS). It appears you are running from a file:// URI — serve the page over HTTPS or use localhost.';
        }
      } catch (_e) {
        /* empty */
      }
      ...
      if (
        ...
        unsupportedReason =
          'Drives requires a secure context (HTTPS). Serve over HTTPS or use localhost.';
      }
      ...
      if (
        ...
        unsupportedReason = 'Drives cannot run in a sandboxed frame.';
      }
      ...
      if (!unsupportedReason && !this._supportsOPFS()) {
        ...
        unsupportedReason = 'Drives requires OPFS support.';
      }
    ...
    _handleError(e) {
      ...
      this._warn('Drives error:', errObj);
      return this.lastError;
    }

    _log(...args) {
      if (!this.DrivesLogEnabled) return;
    ...
    _traverseRAM(volName, path, options = {}) {
      ...
      for (let i = 0; i < targetLen; i++) {
        ...
        if (current.type !== 'dir')
          throw new Error(
            `TYPE_MISMATCH: ${parts.slice(0, i).join('/') || '/'} is not a directory`,
          );
    ...
    async _persistOPFSMetadata(volName) {
      ...
      const metaFileHandle = await volHandle.getFileHandle('.kx_metadata', {
        create: true,
      });
    ...
    async _resolveOPFSNode(volName, relPath, options = {}) {
      ...
      try {
        ...
        try {
          return {
            handle: await current.getDirectoryHandle(last, {
              create: !!options.createDirs,
            }),
            type: 'directory',
          };
        } catch (_e2) {
          ...
          const next = prev
            ...
            .then(async () => {
              ...
              if (!isNew) {
                ...
                if (!this._getPerms(volName, parentRelPath).create)
                  throw new Error(
                    'PERMISSION_DENIED: Create permission denied on parent directory',
                  );
                if (vol.fileCount + 1 > vol.fileCountLimit)
                  throw new Error('QUOTA_EXCEEDED: File count limit reached');
              }
              ...
              try {
                ...
                if (_e.name === 'TypeMismatchError')
                  throw new Error('TYPE_MISMATCH: Target is likely a directory', {
                    cause: _e,
                  });
                throw _e;
              }
    ...
    async exportVolume(args) {
      ...
      try {
        ...
        for (const volName of volsToExport) {
          if (!this.volumes[volName])
            throw new Error(`NOT_FOUND: Volume ${volName} does not exist`);
          if (!this._getPerms(volName, '').control)
          ...
          const processNode = async (name, nodeData, currentPath) => {
            ...
            if (nodeData.type === 'dir') {
              if (childRelPath) {
                ...
                if (this.volumes[volName].type === 'RAM') {
                  this._traverseRAM(volName, childRelPath, {
                    createDirs: true,
                    parentOnly: false,
                  });
                } else {
    ...
    async restoreSnapshot(args) {
      ...
      try {
        ...
        const importResult = await this.importVolume({
          VOL: volName,
          JSON: byVol.get(snapName),
        });
        const status = JSON.parse(importResult);
        if (status.status !== 'success')
    ...
    async runIntegrityTest() {
      ...
      try {
        ...
        try {
          ...
          // 4. Read
          assert(
            (await this.fileRead({ PATH: vol + 'f1.txt', FORMAT: 'text' })) === 'hello',
            'Read',
          );
    ...
    async mountArchive(args) {
      ...
      try {
        ...
        try {
          ...
          throw new Error(`INVALID_ARGUMENT: Invalid JSON — ${e.message || String(e)}`, {
            cause: e,
          });
        }

        if (!data || typeof data !== 'object' || Array.isArray(data)) {
          throw new Error('INVALID_ARGUMENT: JSON must be an object in Drives export format');
        }
        ...
        if (Object.prototype.hasOwnProperty.call(data, volName)) {
          ...
          if (keys.length === 0) throw new Error('INVALID_ARGUMENT: JSON contains no drives');
          volData = data[keys[0]];
        }
  ...
  Scratch.extensions.register(new DrivesExtension());
})(Scratch);
